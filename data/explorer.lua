...
local grid_size = 1.0            -- Size of grid cells in meters
...
local function find_partial_path(start, goal)
    local path = {}
    local current = start
    local max_steps = 100  -- Limite para evitar loops infinitos
    
    for i = 1, max_steps do
        local next_point = get_next_walkable_point(current, goal)
...
local function get_next_walkable_point(current, goal)
    local direction = {
        x = goal:x() - current:x(),
        y = goal:y() - current:y(),
        z = goal:z() - current:z()
    ...
    local next_point = vec3:new(
        current:x() + direction.x * grid_size,
        current:y() + direction.y * grid_size,
        current:z() + direction.z * grid_size
    )
...
local function move_to_target()
    if target_position then
        ...
        if not current_path or #current_path == 0 or path_index > #current_path or 
           (current_time - last_path_calculation_time > path_recalculation_interval) then
            ...
            if raw_path then
                ...
                console.print("No path found to target. Trying partial path.")
                ...
                if partial_path and #partial_path > 0 then
                    ...
                    console.print("Moving to last walkable point before recalculating.")
                else
                    console.print("No partial path found. Disabling explorer.")
                    disable()
                    return
                end
            end
        ...
        if next_point and next_point.x and not next_point:is_zero() and calculate_distance(player_pos, next_point) < grid_size then
            ...
            -- Se chegamos ao final do caminho parcial, recalculamos
